...
db_url <- function(path) paste0("localhost:7300/", path)
...
viruses %>%
  jsonlite::toJSON() %>%
  write("export-new-virus.json")
all_tables_fix_pids %>%
  select(-path) %>%
  jsonlite::toJSON() %>%
  write("export-new-serology.json")
